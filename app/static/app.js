...
Strip: ${led.strip_class || '-'}`;
  ...
  if (metricsEl) {
    metricsEl.innerText = [
      ...
      `Frame Write Timeouts: ${formatNumber(serial.frame_write_timeouts)} | Retry-OK: ${formatNumber(serial.frame_write_timeout_retry_successes)}`,
...
function renderBackendStatusDebug(payload) {
  ...
  summaryEl.innerText = [
    ...
    `LED Write: ${formatMs(display.last_led_write_ms, 3)} | frame sent=${display.last_led_frame_sent === true ? 'ja' : display.last_led_frame_sent === false ? 'nein' : '-'}`,
