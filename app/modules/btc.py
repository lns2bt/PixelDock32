...
class BTCModule(ModuleBase):
    ...
    async def render(self, settings: dict, cache: dict) -> ModulePayload:
        ...
        if price is None:
            frame, colors = render_text_with_colors(
                "B...K",
                char_colors=[(255, 140, 0)] + [(120, 120, 120)] * 4,
                font_size=font_size,
            )
            return ModulePayload(text="B...k", frame=frame, color_frame=colors, font_size=font_size)
        ...
        frame, colors = render_text_with_colors(text, char_colors=char_colors, font_size=font_size)
