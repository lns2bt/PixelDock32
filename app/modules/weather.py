...
class WeatherModule(ModuleBase):
    ...
    async def render(self, settings: dict, cache: dict) -> ModulePayload:
        ...
        if temp is None:
            frame, colors = render_text_with_colors("...C", char_colors=[(120, 120, 120)] * 4, font_size=font_size)
            return ModulePayload(text="...C", frame=frame, color_frame=colors, font_size=font_size)

        value = float(temp)
        text = f"{value:.1f}C"
        color = temperature_to_rgb(value)
        frame, colors = render_text_with_colors(text, char_colors=[color] * len(text), font_size=font_size)
        return ModulePayload(text=text, frame=frame, color_frame=colors, font_size=font_size)
